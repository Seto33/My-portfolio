export function getElements(char, item) {
	Object.entries(item).forEach(([key, value]) => {
		...
		if (elem && char === 'header' && 'links' in item) {
			elem.innerHTML = ""
			item.links.forEach((item) => {
				elem.innerHTML += `
				<li class="nav__item" data-nav-item>
					<a href="${item.link}" class="nav__link link title">
						${item.text}
					</a>
				</li>`
			})
		...
		if (elem && char === 'skills' && key === 'list') {
			elem.innerHTML = ""
			item.list.forEach((item) => {
				elem.innerHTML += `
					<li class="skills__item">
						<img class="skills__img" src="${item?.img?.url}" alt="" width="100" height="100">
						<h3 class="skills__item-title">${item.title}</h3>
						<svg class="skills__rating" data-rating="${item.rating}" width="168" height="21" viewBox="0 0 168 21" fill="none" xmlns="http://www.w3.org/2000/svg">
							<path
		...
		if (elem && char === 'portfolio' && key === 'list') {

			elem.innerHTML = ""
			item.list.forEach((item) => {
				elem.innerHTML += `
				<li class="slider__slide portfolio__slide">
					<img class="portfolio__slide-img" src="${item?.img?.url}" alt="" width="300" height="300">
		...
		if (elem && char === 'contacts' && key === 'list') {
				...
				elem.innerHTML += `
					<li class="social__item">
						<a class="social__link" href="${item.link}">
							<img class="social__img" src="${item?.img?.url}" alt="" width="20" height="20">
						</a>
					</li>
				`
			})
